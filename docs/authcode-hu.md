...
15. [Szinergia a Quench-RAM-mal és a Neuron OS-sel](#szinergia)
...
│ KÜLSŐ VILÁG                                                     │
│                                                                 │
│  Fejlesztő          Neuron OS HSM Card           CIL bytecode       │
│     │                     │                       │             │
│     └──── sign(hash) ─────┘                       │             │
│                     │                             │             │
│                     ▼                             │             │
...
BitIceCertificateV1 (compact 71 byte):
  ...
  IssuerId     (16)   : a kibocsátó delegate ID-je (Neuron OS Vendor)
...
A hash-alapú séma stateful-sága **pozitív tulajdonság** a CFPU kontextusban, mert a Neuron OS HSM Card fizikailag biztosítja a single-use kényszerítést (lásd [Neuron OS HSM Card](#neuroncard)).
...
[BitIce CFPU Foundation Root CA]                      ← HSM-ben tartott
        ...
        ▼
[Neuron OS Vendor Delegate Cert]                      ← vendor (pl. FenySoft)
        │ signs WOTS+ (HSS level 2)
        ▼
[Neuron OS HSM Card]                                      ◄── FEJLESZTŐI KÁRTYA
   └─ Neuron OS HSM Applet                         ← fizikai SE, single-use leaf
...
| Neuron OS HSM Card | fejlesztő kezében | tamper-resistant smart card, single-use NVRAM counter |
...
A Neuron OS HSM Card egy dedikált, dedikált, single-purpose HSM smart card, amely **JavaCard runtime-on** egy különálló **Neuron OS HSM Applet**-et futtat. Ez a kártya **kötelező** minden fejlesztő számára, aki CFPU-ra aktort telepít.
...
A Neuron OS HSM Card tamper-resistant smart card hardvere biztosítja:
...
A JavaCard 3.x+ runtime **firewall mechanizmust** ad az appletek között — egy applet nem fér hozzá egy másik applet adataihoz. Egy különálló **Neuron OS Applet** a BitIce ökoszisztémában:

| Előny | Mit jelent |
|-------|-----------|
| Domain isolation | Bug egy identity / payment applet-ben nem szivárogtat Neuron OS állapotot |
| Független leaf-counter | A Neuron OS HSS-tér csak CIL-aláírásra fogy |
...
| Független élettartam | A kártya cserélhető a Neuron OS oldalon anélkül, hogy más BitIce funkciók érintettek |
...
4. Neuron OS HSM Card csatlakoztatva:
...
Ha egy Neuron OS HSM Card elvész, ellopódik, vagy a privát kulcsa valamilyen módon kompromittálódik, a hozzá tartozó cert-eket **vissza kell vonni**.
...
Az AuthCode + CodeLock + Neuron OS HSM Card együtt **nyolc** klasszikus támadási osztályt zár ki hardveresen:
...
| Stateful sig key reuse | — | Ha szoftveres signer, könnyű | **Kizárva** (Neuron OS HSM Card single-use NVRAM) |
| Quantum break of signature | — | Shor töri ECDSA/Ed25519-et | **Kizárva** (WOTS+/LMS hash-alapú PQC) |

## Szinergia a Quench-RAM-mal és a Neuron OS-sel <a name="szinergia"></a>
...
4. **Neuron OS HSM Card AID** — regisztrált ISO/IEC AID a Neuron OS RID alatt (pl. `<RID>+"NEURONOS"`).
...
7. **Server-side Neuron OS HSM** — PCIe vagy network appliance a CI/CD pipeline-okhoz.
...
| Fázis | AuthCode + CodeLock + Neuron OS HSM Card szerepe |
|-------|------------------------------------------------|
...
| F4 (multi-core szim) | Teljes szoftveres AuthCode flow, BitIce library integrálva a szimulátorba; **szimulált Neuron OS HSM Card** unit tesztekhez |
| **F5 (RTL prototípus)** | Első hardveres SHA-256 + WOTS+ verifier; **valódi Neuron OS HSM Card** prototípus (dev kit); a Neuron OS HSM Applet első verziója |
| F6 (ChipIgnite tape-out) | Teljes hardveres AuthCode + CodeLock + eFuse; production Neuron OS HSM Card elérhető fejlesztőknek |
...
|--------|-------|-------------|
| 1.0 | 2026-04-16 | Kezdeti vízió-szintű kiadás. AuthCode (load-time verify) + CodeLock (W⊕X hardveres) + BitIce WOTS+/LMS integráció + Neuron OS HSM Card (dedikált JavaCard applet). A részletes paraméterek (AID, form factor, HSS mélység, PIN policy, revocation, CI/CD HSM) F-fázis-függő nyitott kérdések. |
