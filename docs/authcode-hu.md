...
│ KÜLSŐ VILÁG                                                     │
│                                                                 │
│  Fejlesztő          Neuron OS Card           CIL bytecode       │
│     │                     │                       │             │
│     └──── sign(hash) ─────┘                       │             │
│                     │                             │             │
│                     ▼                             │             │
...
A hash-alapú séma stateful-sága **pozitív tulajdonság** a CFPU kontextusban, mert a BitIce kártya fizikailag biztosítja a single-use kényszerítést (lásd [Neuron OS Card](#neuroncard)).
...
[Neuron OS Vendor Delegate Cert]                      ← vendor (pl. FenySoft)
        ...
        ▼
[Neuron OS Card]                                      ◄── FEJLESZTŐI KÁRTYA
   └─ BitIce Neuron OS Applet                         ← fizikai SE, single-use leaf
...
| Neuron OS Card | fejlesztő kezében | tamper-resistant smart card, single-use NVRAM counter |
...
A Neuron OS Card egy dedikált, single-purpose BitIce smart card, amely **JavaCard runtime-on** egy különálló **BitIce Neuron OS Applet**-et futtat. Ez a kártya **kötelező** minden fejlesztő számára, aki CFPU-ra aktort telepít.
...
A Neuron OS Card tamper-resistant smart card hardvere biztosítja:
...
4. Neuron OS Card csatlakoztatva:
...
Ha egy Neuron OS Card elvész, ellopódik, vagy a privát kulcsa valamilyen módon kompromittálódik, a hozzá tartozó cert-eket **vissza kell vonni**.
...
Az AuthCode + CodeLock + Neuron OS Card együtt **nyolc** klasszikus támadási osztályt zár ki hardveresen:
...
| Stateful sig key reuse | — | Ha szoftveres signer, könnyű | **Kizárva** (Neuron OS Card single-use NVRAM) |
...
4. **Neuron OS Card AID** — regisztrált ISO/IEC AID a BitIce RID alatt (pl. `<RID>+"NEURONOS"`).
...
7. **Server-side BitIce HSM** — PCIe vagy network appliance a CI/CD pipeline-okhoz.
...
| Fázis | AuthCode + CodeLock + Neuron OS Card szerepe |
|-------|------------------------------------------------|
...
| F4 (multi-core szim) | Teljes szoftveres AuthCode flow, BitIce library integrálva a szimulátorba; **szimulált Neuron OS Card** unit tesztekhez |
| **F5 (RTL prototípus)** | Első hardveres SHA-256 + WOTS+ verifier; **valódi Neuron OS Card** prototípus (dev kit); a BitIce Neuron OS Applet első verziója |
| F6 (ChipIgnite tape-out) | Teljes hardveres AuthCode + CodeLock + eFuse; production Neuron OS Card elérhető fejlesztőknek |
...
|--------|-------|-------------|
| 1.0 | 2026-04-16 | Kezdeti vízió-szintű kiadás. AuthCode (load-time verify) + CodeLock (W⊕X hardveres) + BitIce WOTS+/LMS integráció + Neuron OS Card (dedikált JavaCard applet). A részletes paraméterek (AID, form factor, HSS mélység, PIN policy, revocation, CI/CD HSM) F-fázis-függő nyitott kérdések. |
