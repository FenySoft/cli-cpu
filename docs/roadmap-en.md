...
| — F2.7.E | Sub5.A — config-flash app base offset (parameterizable `CODE_BASE_OFFSET`) | ~3 | ~3 | ✅ DONE (RTL root fix: generic qspi_controller ← core ← a7lite_top ← board; sim default 0 bit-identical parity, FPGA 0xC00000 above the ~9.9 MB bitstream; 2 new cocotb tests test_30/31 + 7 regression targets green; single source generic = write_cfgmem = OpenXC7) |
...
**F2.7 Sub5.A DONE — config-flash app base offset (2026-05-17):** The Sub5 build infra left an "open HW question": the Xilinx SPI master boots the `.bit` bitstream from IS25L128F config-flash 0x0 (~9.9 MB XC7A200T), while the `cilcpu_qspi_controller` read the CODE segment hard-wired also from 0x0 → **bitstream ↔ app collision**. **Root fix (not a workaround):** parameterizable `CODE_BASE_OFFSET` generic in the QSPI controller (`r_addr <= CODE_BASE_OFFSET[23:0] + {4'h0, cpu_addr[19:0]}`), threaded through the full instantiation chain (`cilcpu_qspi_controller` ← `cilcpu_core` ← `cilcpu_a7lite_top` ← `cilcpu_a7lite_board`). SIM default `0` → bit-identical cocotb flash parity (every existing test unchanged); FPGA `0xC00000` (12 MB, above the ~9.9 MB bitstream; app window 1 MB `0xC00000..0xCFFFFF` < 16 MB IS25L128F). **Single source:** the generic = `write_cfgmem.tcl` `CODE_BASE_OFFSET_HEX` = OpenXC7 `chparam` = Vivado `set_property generic`. `SEG_DATA` intentionally not offset (int-only CIL-T0 uses no static DATA flash region; separate task if needed). **TDD:** 2 new cocotb tests (`test_30_code_base_offset_applied`, `test_31_code_base_offset_addr_zero`) RED→GREEN on the `test_qspi_controller_offset` target (`CODE_BASE_OFFSET=256`); regression all green: test_qspi_controller 31/31, test_core 49/49, test_stack_cache 28/28, test_a7lite_top 8/8, test_a7lite_fib 1/1, test_a7lite_board 4/4, test_a7lite_board_fib 1/1 (Fib(20)=6765). Vault: `project_flash_base_offset`.

**Next substantive step:** **F2.7 Sub5** — Vivado / OpenXC7 build with board.v, `write_cfgmem` flow for the IS25L128F (application binary embedded at `CODE_BASE_OFFSET=0xC00000`, Sub5.A), 50 MHz timing closure (tightening Sub4 constraints), and real A7-Lite bring-up with a UART terminal over CH340.
...
|---------|------|---------|
| 2.2 | 2026-06-01 | **F2.8.6 board layer — `cilcpu_tt_board` + A7-Lite XDC (HW bring-up prep).** Synthesizable board top around `cilcpu_tt_top`: reset-sync (KEY1/AA1), UART RX (U2) → loader, UART TX (V2) ← printer, halt/trap LEDs (M18/N18), and the QSPI wired to the **mole99/qspi-pmod** via the JP1 PMOD header (8× IOBUF). **Key difference from `a7lite_board`: NO STARTUPE2** — the QSPI goes to the Pmod on plain I/O (not the onboard config flash). uio→FPGA (from the website JP1 table + user's adapter mapping): `uio[0..7]` = F13/E13/D14/E16/F14/E14/D15/D16 (GPIO1_0..3 P/N). Power: the Pmod's 3.3V from JP1 pin 29 (VCC_3V3), NOT pin 11 (VCC_5V). GPIO-in tied off in v1; IRQ + GPIO/trace-out not pinned (free: GPIO1_4..12). `+define+CILCPU_SIM_BOARD` split debug ports for Verilator. TDD: `test_tt_board` 2/2 (reset uio_oe per qspi-pmod, board e2e i_uart_rx→run→o_uart_tx "42"+halt LED). Regression: 28/28 targets, 335 tests. Vault: `reference_a7lite_pinout`. Remaining: Vivado/OpenXC7 build + HW bring-up. |
| 2.1 | 2026-06-01 | **F2.8.6 DONE — `cilcpu_tt_top` Tiny Tapeout `tt_um` equivalent wrapper.** `cilcpu_soc` (Nano core + UART loader/boot_ctrl + mailbox/gpio/trace + QSPI) bound to the TT standard 24-pin interface (8 dedicated in + 8 out + 8 bidir) + decimal result printer (UART TX). Pin map **aligned to the `mole99/qspi-pmod` FIXED pinout** (QSPI consumes all 8 uio): `uio[0]`=cs_flash(CS0), `uio[1,2,4,5]`=DQ0-3(SD0-3), `uio[3]`=clk(SCK), `uio[6]`=cs_psram(CS1/RAM A), `uio[7]`=CS2/RAM B (unused→high); `ui_in[0]`=UART RX, `ui_in[7:1]`=GPIO_IN; `uo_out[0]`=UART TX, `[1:3]`=halt/trap/irq, `[7:4]`=trace/gpio-out mux (4-bit). The mailbox host side is NOT pinned (F3: UART protocol). Boot is only via the UART loader (external boot port tied off), like the real F3 chip. TDD: 4 new cocotb tests (`test_tt_top` 4/4 — reset pin direction per qspi-pmod, UART→QSPI→UART e2e, GPIO-out+trace mux, GPIO-in). Regression: 27/27 targets green, 333 tests. This is the functional precursor of the F3 tape-out wrapper. Remaining: A7-Lite board-level XDC (QSPI goes to the qspi-pmod via a PMOD header, plain IOBUF — NOT config-flash/STARTUPE2; needs the JP1/JP2→FPGA pin table) + HW bring-up. ADR: `Decisions/2026-06-01-tt-pin-map`. |
| 2.0 | 2026-06-01 | **F2.8 #6.5b-F2 DONE — QSPI flash erase+program (persistent dev=flash loading).** The `cilcpu_qspi_controller` Flash CODE/DATA segment became writable: a `cpu_we` runs the `WREN → Sector Erase (0x20) → WIP-poll → WREN → Page Program (0x02) → WIP-poll` sequence (SPI 1-1-1). **Auto-erase on sector change** (`r_last_erased_sector`/`r_have_erased`) → buffer-less streaming load erases once per sector, multi-word programs are not corrupted. 5 new FSM states (`ST_FW_*`) + sub-sequencer (`r_fw_step`). TDD: 4 new controller tests (test_32–35: round-trip, multi-word/1-erase, WIP-poll, NOR-erase proof) + SoC-level e2e (`test_soc.test_09`: UART dev=0 → flash → BOOT code_src=0 → run), `test_10` behavior change. Regression: 26/26 targets green (test_qspi_controller 35/35, test_soc 9/9). **Known HW characteristic (not a mine):** flash latency (~45 ms sector erase) ≫ UART byte → with the buffer-less loader the host throttles (`test_09` models this). Boot-load ADR F2 phase closed. Spec: `QSPI_CONTROLLER_SPEC` v1.2. Vault: `f2-flash-write-uart-throttle`. |
