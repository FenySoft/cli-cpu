...
|---------|------|---------|
| 2.1 | 2026-06-01 | **F2.8.6 DONE — `cilcpu_tt_top` Tiny Tapeout `tt_um` equivalent wrapper.** `cilcpu_soc` (Nano core + UART loader/boot_ctrl + mailbox/gpio/trace + QSPI) bound to the TT standard 24-pin interface (8 dedicated in + 8 out + 8 bidir) + decimal result printer (UART TX). Confirmed pin map: `ui_in[0]`=UART RX, `ui_in[7:1]`=GPIO_IN; `uo_out[0]`=UART TX, `[1:3]`=halt/trap/irq, `[7:4]`=trace/gpio-out mux; `uio[3:0]`=QSPI DQ (bidir, oe), `uio[6:4]`=cs_flash/cs_psram/clk, `uio[7]`=mux[4]. The mailbox host side is NOT pinned (F3: UART protocol). Boot is only via the UART loader (external boot port tied off), like the real F3 chip. TDD: 4 new cocotb tests (`test_tt_top` 4/4 — reset pin direction, UART→QSPI→UART e2e, GPIO-out+trace mux, GPIO-in). Regression: 27/27 targets green, 333 tests. This is the functional precursor of the F3 tape-out wrapper. Remaining: A7-Lite board-level XDC (STARTUPE2/IOBUF) + HW bring-up. |
