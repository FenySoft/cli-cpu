...
|--------|-------|-------------|
| 2.1 | 2026-06-01 | **F2.8.6 KÉSZ — `cilcpu_tt_top` Tiny Tapeout `tt_um` ekvivalens wrapper.** A `cilcpu_soc` (Nano core + UART loader/boot_ctrl + mailbox/gpio/trace + QSPI) bekötve a TT szabványos 24-pin interfészére (8 dedikált in + 8 out + 8 bidi) + decimális eredmény-printer (UART TX). Megerősített pin-map: `ui_in[0]`=UART RX, `ui_in[7:1]`=GPIO_IN; `uo_out[0]`=UART TX, `[1:3]`=halt/trap/irq, `[7:4]`=trace/gpio-out mux; `uio[3:0]`=QSPI DQ (bidir, oe), `uio[6:4]`=cs_flash/cs_psram/clk, `uio[7]`=mux[4]. A mailbox host-oldal NINCS pin (F3: UART-protokoll). A boot kizárólag UART-loaderen át (külső boot-port tied-off), mint a valós F3 chipen. TDD: 4 új cocotb teszt (`test_tt_top` 4/4 — reset pin-irány, UART→QSPI→UART e2e, GPIO-out+trace mux, GPIO-in). Regresszió: 27/27 target zöld, 333 teszt. Ez az F3 tape-out wrapper funkcionális precursor-a. Hátra: A7-Lite board-szintű XDC (STARTUPE2/IOBUF) + HW bring-up. |
