# CFPU Secure Edition — Nyílt Secure Element / TEE / JavaCard utóda
...
> *A **CFPU Secure Edition** egy chip-termékcsalád a [CFPU](brand-hu.md) kategórián belül. A nyílt forráskódú referencia-implementáció (szimulátor, linker, ISA spec, RTL) a **CLI-CPU** projekté. Ebben a dokumentumban a „CFPU Secure Edition" a szilícium-szintű termékcsaládot jelenti; a „CLI-CPU" csak ott szerepel, ahol a projektre hivatkozunk (roadmap fázis, szimulátor, linker output). A teljes elnevezés-rendszer: [brand-hu.md](brand-hu.md).*
...
5. [CFPU Secure Edition — pozicionálás és megkülönböztetés](#pozicionalas)
...
Az eddigi `docs/architecture-hu.md`, `docs/security-hu.md` és [`Symphact/docs/vision-hu.md`](https://github.com/FenySoft/Symphact/blob/main/docs/vision-hu.md) dokumentumok **két piaci pályát** rögzítettek:
...
2. **Tökéletes architekturális illeszkedés.** A CFPU alap-tulajdonságai (memory safety, shared-nothing, formálisan verifikálható ISA, kicsi méret, alacsony fogyasztás, determinizmus) **pontosan** azok, amit a Secure Element piac keres
3. **Megkülönböztető pozicionálás lehetséges.** A jelenlegi nyílt SE projektek (TROPIC01, OpenTitan) **mind** egymagos, hagyományos modelleket használnak. A CFPU multi-core, shared-nothing, aktor-alapú megközelítése **új kategóriát** teremthet: több független Secure Element egy chipen, hardveres isolation-nel
...
A **JavaCard** (Oracle, eredetileg Sun Microsystems, 1996-ban jelent meg) a világ legelterjedtebb smart card szoftver platformja. Becslések szerint **~30 milliárd kártyán** fut a világon: SIM-ek, bankkártyák, nemzeti eID, tömegközlekedési jegyek, eSIM-ek, FIDO authenticator-ek. A JavaCard **pontosan** az, amit a CFPU is próbál — **bytecode natív végrehajtás** izolált biztonsági hardveren — csak:
...
A probléma közös: a TEE-ket **utólag építik** egy shared-memory, speculative-execution CPU-ra. A CFPU ezzel szemben **eleve shared-nothing**, tehát **nincs utólag hozzáadandó isolation réteg** — ez a rendszer alapja.
...
A TROPIC01 **nem csak egy koncepció vagy prototípus** — **2026 Q1-ben full production**, már rendelhető, és az első partner termékek (Trezor hardware wallet új generációja, MikroE Secure Tropic Click) már elérhetők. Ez az **első** valóságos példa arra, amit a CFPU Secure Edition is céloz: **nyílt architektúrájú, auditálható secure element**.
...
7. **Fejlesztési időkeret**: ~6 év alapítástól production-ig. Ez reális cél a CFPU Secure Edition-nek is.
8. **SatoshiLabs partnership**: a Bitcoin/crypto közösség **hajlandó pénzt adni** nyílt hardverért, ha a narratíva jó.

## CFPU Secure Edition — pozicionálás és megkülönböztetés <a name="pozicionalas"></a>

### Nem versenytárs, hanem következő generáció

A TROPIC01 **kiváló első generációs nyílt secure element**, és az OpenTitan is a maga datacenter RoT szegmensében erős. Ezek **kihúzzák az utat** a CFPU Secure Edition számára — bizonyítják, hogy a piac létezik, érett, és pénzeli a nyílt alternatívákat.

**A CFPU Secure Edition nem ezek ellen versenyzik.** Egy **következő generációs** megközelítést kínál, ami **más architektúrán alapul**, és a piac **még nem megoldott** problémáira ad választ:

| Szempont | TROPIC01 | OpenTitan | CFPU Secure Edition |
|----------|----------|-----------|------------------------|
...
A CFPU Secure Edition **egyetlen chipen több független Secure Element**-et kínál, **hardveres shared-nothing** izolációval. Minden Nano core **egy független secure applet**, amely **matematikailag bizonyítottan** nem tud kommunikálni a többivel, csak a definiált mailbox interfészeken át.
...
**CFPU Secure Edition-nel:** egy chip, **7 külön Nano core**, mindegyik egy teljes aktor-alapú Secure Element, **Rich core** supervisor koordinálja. Ha a banking core valaha kompromittálódna, a többi **architekturálisan érintetlen** — fizikai, hardveres garancia, nem szoftveres remény.

**Ez egy valós, megoldatlan piaci probléma**, amit a CFPU Secure Edition **meg tud oldani**.

## Architekturális illeszkedés — mi adott, mi hiányzik <a name="architekturalis-illeszkedes"></a>

### Amit a jelenlegi CFPU terv **már tartalmaz**
...
| Secure Element követelmény | CFPU status | Fázis |
|---------------------------|----------------|-------|
...
| **Capability-based security (aktor ref)** | ✓ | F5 (Symphact) |
| **Supervision hierarchia (fault tolerance)** | ✓ | F5 (Symphact) |
...
A TROPIC01 SPECT mintáját átvéve, a CFPU Secure Edition-be egy **dedikált Crypto Actor** kerül be. Ez egy külön Nano core-szerű egység, **de saját mikrokódolt instrukciókkal** a kriptográfiai workload-ra optimalizálva.
...
Ez lehetővé teszi a **remote trust establishment**: egy felhő szerver **megerősítheti**, hogy a kommunikáló eszköz **valóban** egy valódi, auditált CFPU Secure Edition.

### 6. Tamper Detection

A TROPIC01-ből tanulva, a CFPU Secure Edition **ugyanezt a csomagot** kell hogy tartalmazza:
...
| Komponens | Std cell |
|-----------|----------|
| Alap CFPU (Nano + Rich, F6 terv) | ~200k |
...
**A CFPU Secure Edition reálisan EAL-5+ célzott**, és **EAL-6+** is elérhető a formálisan verifikálható ISA és az egyszerű mikroarchitektúra miatt.
...
Ez **nem kezdő projekt költségkeret**. Az EAL-5+ tanúsítást a CFPU Secure Edition **F6.5 után** (tehát 2031-2032 körül) érdemes elindítani, amikor már van valódi szilícium és tapasztalat.
...
A Tropic Square jelenleg a **BSI** és **ANSSI** felé mozdul. A CFPU Secure Edition ugyanezen az úton haladhat, **tanulva a TROPIC01 tapasztalataiból**.
...
A CFPU Secure Edition **egy termékcsalád**, amely **több tanúsítást szerez** a különböző célterületekre.

## Konkrét termékcsalád és használati módok <a name="termekcsalad"></a>

A CFPU Secure Edition **nem egy chip**, hanem **egy platform**, amelyre **több konkrét termék** épülhet, minden egyes termék egy-egy piaci szegmenset céloz. A **hardveres alap közös**, de a **firmware és a tanúsítás** különböző.

### 1. CFPU Open Banking Card (EMV-kompatibilis)
...
**Megkülönböztető**: **privacy-preserving medical AI inference, tanúsítható device identity, post-quantum ready crypto**. Jelenleg az orvosi eszközök többnyire **elavult 8-bites MCU-k**, mert az új CPU-kat nem tudják tanúsítani. A CFPU modern és tanúsítható.
...
**Megkülönböztető**: a TROPIC01 **egy** security domain-t ad; a CFPU Secure Edition **több** crypto-t támogat egyszerre (Bitcoin + Ethereum + Solana + monero + ...), mindegyiket külön hardveres security domain-ben. **Multi-chain multi-wallet egyetlen chipen**, biztonságosan.
...
**Cél**: egy kis, formálisan verifikált CFPU Secure Edition **felügyeli** egy nagy AI modell (LLM, agent) döntéseit, és vészleállítja, ha anomáliát észlel.
...
- **Hardveres partnerség**: egy TROPIC01-et akár **használhatunk** egy CFPU chip mellett root of trust-ként
...
A SatoshiLabs finanszírozza a Tropic Square-t. Ha a CFPU Secure Edition egy **multi-chain hardware wallet** változatot kínál (ami 10. terméktípus), akkor a **Trezor ökoszisztéma** egy természetes partner.
...
A CFPU Secure Edition **nem egy gyors projekt**. A Tropic Square tanulsága: **~6 év** az alapítástól a full production-ig, és **2-3 év** a Common Criteria tanúsításig.
