...
**A kulcs szám:** a RISC-V **a chip területének 10-20%-át a cache coherency-re költi**. A CLI-CPU-n ez a terület **extra core-nak megy**, mert a shared-nothing architektúrában a koherencia probléma **nem létezik**. Ezért fér el 23 core 15 mm²-en, míg a RISC-V 4-8 core-nál megáll. A RAM mennyiségben hasonlóak (~150 KB), de a CLI-CPU-é **privát** (nincs coherency traffic), a RISC-V-é **megosztott** (coherency lassítja).

### Teljesítmény-összehasonlítás actor-alapú workload-okon

| Metrika | CLI-CPU (6R+16N @50MHz) | RISC-V 4-core (@50MHz, same die) | CLI-CPU előny |
|---------|------------------------|----------------------------------|---------------|
