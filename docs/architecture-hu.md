...
> *Ebben a dokumentumban a „**CFPU**" a chip-szintű architektúrára vonatkozik (mikroarchitektúra, pipeline, memória térkép, biztonsági primitívek, core típusok) — ez a *kategória*, testvér a CPU/GPU/TPU/NPU mellett. A „**CLI-CPU**" a nyílt forráskódú *projekt*, ami megvalósítja (F0–F7 roadmap fázisok, szimulátor, linker, ISA spec, GitHub repo). Ahol egy mondat a szilíciumról szól, használjuk a CFPU-t; ahol fázisról, build artefaktumról vagy teszt célról, használjuk a CLI-CPU-t. A teljes elnevezés-rendszer: [brand-hu.md](brand-hu.md); a rövid változat a [FAQ #1](faq-hu.md#1-mi-a-cfpu-és-mi-a-kapcsolata-a-cli-cpu-val).*
...
| **CFPU (Cognitive Fabric)** | **✓ teljes CIL** | **✓** | **✓** | **✓** | **✓** (hw mailbox wake) | **✓** (CIL stack-gép) |
...
**A CFPU az egyetlen pozíció**, ahol **mind a hat oszlop** teljesül: programozható csomópont + hardveres + nyílt + .NET natív + event-driven + stack-kompakt ISA. Ez nem csak „egy újabb bytecode CPU", hanem **egy új kategória**.

### Neuromorphic-ihletett, de nem neuromorphic

A CFPU **átveszi** a neuromorphic architektúrák legértékesebb elveit:
...
A CFPU **nem C#-t futtat — CIL-t futtat**. Az ECMA-335 Common Intermediate Language az a célformátum, amelyre **minden .NET nyelv** fordít. Ez azt jelenti, hogy a CFPU natívan futtatja:

| Nyelv | Paradigma | CFPU illeszkedés |
|-------|-----------|-------------------|
...
**Ez ~8 millió fejlesztő meglévő kódbázisa**, amely natívan futhat a CFPU-n — fordítás, interpreter, vagy runtime overhead nélkül.
...
#### Összehasonlítás: RISC-V vs CFPU .NET alkalmazásokon

Ugyanazon a ~10 mm² Sky130 szilíciumon:

| Metrika | CFPU (10R+8N) | RISC-V 4-core + CIL interpreter | RISC-V 4-core + AOT |
|---------|------------------|--------------------------------|---------------------|
...
A RISC-V-nek **két rossz opciója** van .NET kódra: interpreter (10-50× lassabb) vagy AOT (10-50× nagyobb bináris, I-cache pressure). A CFPU **natívan futtatja a CIL-t**, kompakt formában, hardveres actor támogatással.

**A narratíva:** A CFPU **nem „még egy CPU"** — hanem **az első hardver platform, ami egy 8 milliós fejlesztői ökoszisztéma natív szilíciumát adja**. Minden C#, F#, VB.NET kód, ami CIL-re fordul, natívan fut a Cognitive Fabric-on — átírás, interpreter, vagy JIT nélkül.
...
3. **Harvard modell külső memóriával.** Külön kód-flash és külön PSRAM adatnak (F3–F5: QSPI, CF One: megosztott OPI busz). A chip-en belüli SRAM kizárólag gyorsítótár.
...
A CLI-CPU projekt F4 fázisában a CFPU először válik **valódi hálózattá**. A 4 core egymástól függetlenül futtat saját CIL programot, mindegyik saját lokális SRAM-mal, és kizárólag a mailbox interfészeken keresztül kommunikál. Nincs megosztott heap, nincs cache koherencia:
...
A CLI-CPU projekt F5 fázisától a CFPU **heterogén multi-core** architektúrára vált, három bevált ipari koncepciót egyesítve egyetlen chipben:
...
- **Intel Alder Lake (2021):** P-core (Performance) + E-core (Efficiency) heterogén keverék, az OS ütemezőre bízva a feladatkiosztást. **Nálunk: a Symphact supervisor osztja ki a feladatokat Rich és Nano core-ok között.**
...
| F6-Silicon One | ChipIgnite Sky130 (15 mm², ~$15K) | **23** | **2** | **„Cognitive Fabric One"** — teljes demonstráció, benchmark |
...
A Nano és Rich mellett a CFPU egy **harmadik, infrastruktúra jellegű core típust** tartalmaz: a **Secure Core**-t. Ez **nem számítási core** (nem fut rajta felhasználói kód), hanem a rendszer **trust anchor-ja** — minden betöltendő kód rajta keresztül megy.
...
Egy core típust az **ISA-ja** definiál (milyen opkódokat tud végrehajtani), nem a cache vagy SRAM mérete — az csak konfiguráció. Elméletileg lehetne „Micro" (még kisebb ISA, csak 16 opkód) is, de ez **bonyolítja a programozási modellt**: a fejlesztőnek három különböző opkód-készletre kellene figyelnie. A kereskedelmi példák (Apple, ARM, Intel) **mind** pontosan 2 számítási core típust használnak, és ez a sweet spot. A CFPU is **2 számítási core-nál** marad (Nano és Rich), kiegészítve a Secure Core infrastruktúra elemmel.
...
- **Nincs felesleges RISC-V management core** — a CFPU-nak saját Rich core-jai vannak, nem kell külső CPU
...
| **Maradék** | | | **~5.27 mm²** (tartalék extra SRAM-ra, routing/timing closure-hoz, vagy jövőbeli bővítésre) |

### Külső memória interfész

A 160 KB on-chip SRAM a core-ok lokális cache-ének elég, de a program kód és a nagyobb adatstruktúrák **külső memóriáról** jönnek. A Sky130 I/O cellák ~50-100 MHz SDR-re képesek — ez meghatározza az interfész választást:
...
A **6R+16N+1S** a referencia konfiguráció, de az RTL **paraméterezhető** (`#NUM_RICH`, `#NUM_NANO`). Ugyanaz a design különböző arányokkal instanciálható a célpiac szerint:
...
| **6R + 16N + 1S** | **6** | **16** | **1** | **23** | **Referencia — alkalmazás + demó egyensúly** |
...
### Miért éppen a 6R+16N+1S a referencia

**Az SRAM a terület-domináns elem, nem a logika.** A 6 Rich + 16 Nano + 1 Secure core logikája összesen ~2.06 mm² (6×0.25 + 16×0.031 + 0.06) — a 15 mm²-es chipnek ez csak ~14%-a. A maradék ~79% SRAM és routing. Ez azt jelenti:
- A core-ok **olcsók** (egy Nano core logikája ~0.031 mm²)
- A memória **drága** (16 KB SRAM ~0.5 mm²)
- A **sweet spot** a 4 KB/Nano + 16 KB/Rich — elég a TOS cache, lokális változók és frame-ek on-chip tartásához, a nagy adat OPI PSRAM-ról jön (megosztott OPI busz)

A maradék ~5.3 mm² felhasználása (F6-Silicon döntés):
...
**Összehasonlítás a CFPU Cognitive Fabric One-nal:**

| | **CFPU 6R+16N+1S** | **RISC-V 4 core** | **RISC-V 8 core** |
|--|----------------------|------------------|------------------|
| **Core-ok** | **23** (6 Rich + 16 Nano + 1 Secure) | **4** | **8** |
| **On-chip RAM** | **160 KB** (privát, koherencia nélkül) | **~214 KB** (L1+L2+extra) | **~152 KB** (L1+L2+extra) |
...
| **Párhuzamos aktorok** | **23** (hw mailbox) | 4 (sw queue + lock) | 8 (sw queue + lock) |
| **Context switch** | **5-8 ciklus** | 500-2000 ciklus | 500-2000 ciklus |

**A kulcs szám:** a RISC-V **a chip területének 10-20%-át a cache coherency-re költi**. A CFPU-n ez a terület **extra core-nak megy**, mert a shared-nothing architektúrában a koherencia probléma **nem létezik**. Ezért fér el 23 core 15 mm²-en, míg a RISC-V 4-8 core-nál megáll. A RAM mennyiségben hasonlóak (~150 KB), de a CFPU-é **privát** (nincs coherency traffic), a RISC-V-é **megosztott** (coherency lassítja).

### Teljesítmény-összehasonlítás actor-alapú workload-okon

| Metrika | CFPU (6R+16N @50MHz) | RISC-V 4-core (@50MHz, same die) | CFPU előny |
|---------|------------------------|----------------------------------|---------------|
| **Actor üzenet/sec** | ~44M (22 core × ~2M/core, hardveres mailbox) | ~2M (szoftveres queue + lock + context switch) | **~22×** |
...
| **Párhuzamos neuronok (SNN)** | 16 (1/Nano core, determinisztikus) | 4 (thread, nem-determinisztikus) | **4×** |
...
**Fontos:** az egy-core IPC-ben a RISC-V (különösen OoO változatban) gyorsabb. A CFPU **nem az egy-core versenyben** nyer, hanem abban, hogy **ugyanazon a szilíciumon sokkal több hasznos, párhuzamos munkát végez** actor-alapú workload-okon, miközben determinisztikus és biztonságos marad.

### Symphact rétegek a chipen
...
|------|---------------|-------------|
| **Actor ping-pong throughput** | 16 Nano pár | Üzenet/sec benchmark — összehasonlítható RISC-V-vel |
| **SNN (Spiking Neural Network)** | 16 Nano (LIF/Izhikevich neuron) + 1 Rich coordinator | Lineáris skálázódás, determinizmus, event-driven energia |
| **IoT edge gateway** | 6 Rich (supervisor + protocol) + 16 Nano (handler) | Valós use-case, latency mérés, fault tolerance demó |
| **Akka.NET actor cluster** | 6 Rich (supervisor) + 16 Nano (worker) | C# kódból fordított actor rendszer, hardveresen futva |
...
> *„A Cognitive Fabric One a világ első nyílt forráskódú, heterogén, actor-natív processzora. 23 core-jával (6 Rich + 16 Nano + 1 Secure), cache coherency nélkül, ugyanazon a 15 mm² Sky130 szilíciumon 22× több actor üzenetet kezel másodpercenként, mint egy hagyományos 4-core RISC-V — miközben determinisztikus, hardveresen izolált, és lineárisan skálázódik. Ez nem gyorsabb CPU — ez egy új paradigma."*

## Blokk diagram (egymagos CFPU, F6 single-core cél)
...
A CFPU **klasszikus 5-fokozatú in-order pipeline-t** használ, a stack-gép szemantikához igazítva:
...
A CFPU ezt három mechanizmussal kezeli:
...
A CFPU 32-bites virtuális címteret használ, logikailag négy régióra osztva:
...
A CFPU-n a CIL **a gépi kód** — nincs JIT, nincs interpreter, nincs köztes fordítás. Amit a Roslyn generál (method header + opkódok), azt a hardver **közvetlenül végrehajtja**. Ezért a frame struktúrát nem a hardver tervező dönti el szabadon, hanem **a Roslyn kimenete rögzíti**:
...
Ez különbözik a hagyományos .NET-től, ahol a JIT szabadon dönthet (regiszterbe teszi a lokálist, átrendezi a frame-et, inline-ol). A CFPU-n ilyen szabadság nincs.
...
A CFPU belső mikro-utasítás formátuma (F2-ben véglegesítendő, előzetes vázlat):
...
A CIL metaadat-tokenek (pl. `MethodDef 0x06000042`) a PE/COFF metaadat táblákba mutatnak. A CFPU-n ezek feloldása a **Metadata Walker koprocesszor** feladata (F4-től):
...
A CFPU nem az első bytecode-natív CPU, és érdemes megtanulni mindegyik elődtől.
...
**Nem elődünk**, de referencia architektúra. A RISC-V egy tisztán regiszter-alapú RISC, ami pont az **ellentéte** a stack-gép CFPU-nak. Viszont:
...
A CFPU **négy power domain**-re osztott (F6 cél):
...
| Kódmegosztás (azonos actor típusok) | **F5+** | Symphact scheduler funkció |
| Actor migráció core-ok között | **F6+** | Cross-core state transfer Seal Core-on át |

## Silicon-grade security

Ez a szekció a CFPU biztonsági architektúráját tárgyalja **az architektúra szemszögéből**. A teljes biztonsági modellt, threat model-t, támadás-immunitási táblázatot, formális verifikáció tervet és tanúsítási útvonalakat külön dokumentum tartalmazza: lásd [`docs/security-hu.md`](security-hu.md).

### A CFPU biztonsági alapelve
...
**Fontos megjegyzés:** az F3 Tiny Tapeout chipen **már az alapvető biztonsági ellenőrzések többsége élesben van**. Ez azt jelenti, hogy a CLI-CPU projekt első valódi szilíciuma **már olyan biztonsági tulajdonságokkal rendelkezik, amikkel egyetlen szabványos CPU sem**.

### Támadás-osztályok, amelyekre immunis a CFPU
...
A CFPU biztonsági profilja a **Cognitive Fabric** (programozható kognitív szubsztrátum) narratíva **mellett** egy második piaci pályát is megnyit:
